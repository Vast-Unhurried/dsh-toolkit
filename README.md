...
| :moneybag: **dsh-api-balance** | API 余额 + 本轮费用 + 今日消耗（¥ / token，峰谷计价，按供应商统计） | 会话头部操作行 + 每条回复操作行 |
...
- **今日消耗**：悬停徽章显示**当日（北京时间）该供应商消耗的 token**，按供应商分别统计，跨会话累计
- **切换联动**：切换会话或切换供应商模型时，徽章立即切换为该供应商的余额与今日消耗；同供应商内切换模型则保持不变
